...
class TestAddDocuments:
    ...
    async def test_ids_are_strings(self, rag, small_docs):
        docs, _ = small_docs
        ids = await rag.add_documents(docs)
        assert all(isinstance(i, str) for i in ids)

    async def test_documents_persisted(self, rag, small_docs):
        docs, _ = small_docs
        await rag.add_documents(docs)
        count = await rag.count_by_metadata(None)
        assert count == len(docs)

    async def test_add_with_labels(self, rag, small_docs):
        """add_documents with DiskANN labels must not raise."""
        docs, labels = small_docs
        ids = await rag.add_documents(docs, labels=labels)
...
class TestAddDocumentsBatch:
    async def test_batch_returns_correct_count(self, rag, medium_docs):
        docs, _ = medium_docs
        ids = await rag.add_documents_batch(docs, batch_size=10, show_progress=False)
        assert len(ids) == len(docs)

    async def test_batch_persists_all_docs(self, rag, medium_docs):
        docs, _ = medium_docs
        await rag.add_documents_batch(docs, batch_size=10, show_progress=False)
        count = await rag.count_by_metadata(None)
        assert count == len(docs)
...
class TestAddDocumentsBatchIsolated:
    ...
    async def test_isolated_continues_on_error(self, rag, small_docs):
...
class TestDeleteDocuments:
    async def test_delete_returns_count(self, rag, small_docs):
        ...
        ids = await rag.add_documents(docs)
        to_delete = ids[:5]
        count = await rag.adelete(to_delete)
        assert count == 5

    async def test_delete_reduces_total(self, rag, small_docs):
        docs, _ = small_docs
        ids = await rag.add_documents(docs)
        original_count = await rag.count_by_metadata(None)
        await rag.adelete(ids[:5])
        new_count = await rag.count_by_metadata(None)
        assert new_count == original_count - 5

    async def test_delete_empty_list(self, rag):
        result = await rag.adelete([])
...
class TestCountByMetadata:
    ...
    async def test_count_with_simple_filter(self, rag, medium_docs):
        docs, _ = medium_docs
        await rag.add_documents(docs)
        active = await rag.count_by_metadata({"status": "active"})
        archived = await rag.count_by_metadata({"status": "archived"})
        assert active + archived == len(docs)
        assert active > 0
        assert archived > 0

    async def test_count_empty_collection(self, rag):
        count = await rag.count_by_metadata(None)
        assert count == 0
