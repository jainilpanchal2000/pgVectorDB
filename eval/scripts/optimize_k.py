...
from pgvectordb.metrics import KValueAnalysis, create_sample_evaluation_dataset


async def test_k_value(db: pgVectorDB, queries: list[str], k: int) -> list[list[str]]:
    """
    ...
    Args:
        ...
        db: Initialized pgVectorDB system
    ...
    for query in queries:
        ...
        docs = await db.semantic_search(query, k=k)
...
async def main():
    ...
    for i in range(100):
        ...
        documents.append(
            Document(
                ...
                metadata={"doc_id": i, "category": category, "year": 2020 + (i % 5)},
            )
        )
    ...
    for k in k_values_to_test:
        ...
        results = await test_k_value(db, dataset.queries, k)
